...
// end 
 ...
 // hongbosherlock: has_act_order (optional)
 ...
 #pragma unroll
   ...
   auto global_reduce = [&](bool first = false, bool last = false) {
       ...
       if (threadIdx.x < active_threads) {
           ...
           c_gl_wr += (4 * thread_n_blocks) * 0; // slice_col assumed 0 for now
 ...
 #pragma unroll
   for (int i = 0; i < thread_m_blocks * 4; i++) {
     if (!first) {
       int4 d_red = sh[c_sh_wr + i * c_sh_wr_delta];
 #pragma unroll
       for (int j = 0; j < 2 * 4; j++) {
         reinterpret_cast<int*>(
             &frag_c)[4 * 2 * 4 * (i / 4) + 4 * j + (i % 4)] +=
             reinterpret_cast<int*>(&d_red)[j];
       }
     }
     if (!last) {
       int4 d;
 #pragma unroll
       ...
       int64_t sorted_row = block_sorted_ids[c_idx / c_gl_stride];
       int64_t true_idx = sorted_row * c_gl_stride + c_idx % c_gl_stride;
       if (c_idx / c_gl_stride < block_num_valid_tokens) C[true_idx] = d;
     }
   }
 };
